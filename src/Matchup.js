	...
	for(let winner in wins[videogameId]) {
		...
		for(let loser in wins[videogameId][winner]){
			...
			if(matches >= MATCHUP_THRESHOLD && winner != loser) {
				...
				if(!(possibleMatchups[videogameId][alphabeticallyFirst].includes(alphabeticallyLast))){
					possibleMatchups[videogameId][alphabeticallyFirst].push(alphabeticallyLast);
				}
			}
		}
	}
...
const Matchup = ({ videogameId, left,right, quizMode }) => {
	...
	const handleRandomMatchup = ({videogameId, leftChar, rightChar}) => {
	...
	useEffect(() => {
		let [winsL, winsR] = getWins(videogameId, leftCharacterName, rightCharacterName);
	...
	return (
		<div className="matchup-container">
			<div className="top-row">
				<LabeledCharacterPortrait videogameId={videogameId} name={leftCharacterName} />
				<MatchupSlider winsL={leftWins} winsR={rightWins} quizMode={quizMode}/>
				<LabeledCharacterPortrait videogameId={videogameId} name={rightCharacterName} />
			</div>
			<div className="bottom-row">
				<MatchupNavigator videogameId={videogameId} leftCharacter={leftCharacterName} rightCharacter={rightCharacterName}/>
