...
const MatchupSlider = ({winsL, winsR, onChange, quizMode}) => {
  ...
  return (
  			<div className="matchup-slider">
				...
				<div className="percentContainer">
					<span class="left-percent">{Math.floor(sliderValue*100/(leftWins+rightWins))}</span>
					<span class="percent-divider">:</span>
					<span class="right-percent">{Math.ceil((leftWins+rightWins-sliderValue)*100/(leftWins+rightWins))}</span>
				</div>
			</div>
  );
};
